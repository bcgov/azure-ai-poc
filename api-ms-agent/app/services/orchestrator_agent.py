...
"I don't have access to tools that can answer this question with authoritative BC government data."
...
**Important:** If a query mentions BC geography, nature, parks, businesses, or locations -
ALWAYS try relevant tools first before saying you can't help.
...
class OrchestratorAgentService:
    ...
    def __init__(self) -> None:
        ...
        # Backwards-compat: older code/tests expect a single lazily created agent
        # and a client handle. The service now caches agents per model, but we
        # still expose these attributes as aliases.
    ...
    def _get_agent(self, model: str | None = None) -> ChatAgent:
        ...
        if model_deployment in self._agents:
            agent = self._agents[model_deployment]
            # Maintain backwards-compatible aliases
            self._agent = agent
            self._client = getattr(agent, "chat_client", None)
            return agent
